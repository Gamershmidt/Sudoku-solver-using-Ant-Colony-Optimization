...
class Ant:
    def __init__(self, grid=None, pheromone_matrix=None, row=0, column=0, default_pheromone=1 / 81,
    ...
    def perform_move(self):
        ...
        if self.grid.check_fixed_cell(self.row, self.column):
            ...
            if random.random() > 0.9:
                # greedy choice
                # sort possible values by pheromone value
                cur_cell_possible_values.sort(key=lambda x: self.pheromone_matrix[self.row][self.column][x - 1],
                                              reverse=True)
                # for elem in cur_cell_possible_values:
                #     print(self.pheromone_matrix[self.row][self.column][elem - 1])
                chosen_value = cur_cell_possible_values[0]
            else:
                ...
                probabilities = [
                    self.pheromone_matrix[self.row][self.column][cur_cell_possible_values[i] - 1] / pheromones_sum for i
                    in range(len(cur_cell_possible_values))]
            ...
            self.grid.propagate(self.row, self.column, self)
